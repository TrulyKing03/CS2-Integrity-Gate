...
switch ($Scenario.ToLowerInvariant()) {
    ...
    "policy-hash" {
        powershell -ExecutionPolicy Bypass -File scripts/smoke-policy-hash.ps1 -Backend $backend
        break
    }
    "security-events" {
    ...
    "security-alerts" {
        powershell -ExecutionPolicy Bypass -File scripts/smoke-security-alert-status.ps1 -Backend $backend
        break
    }
    "security-alerts-manual" {
        powershell -ExecutionPolicy Bypass -File scripts/smoke-security-alert-manual.ps1 -Backend $backend
        break
    }
    "session-revoke" {
    ...
    "queue-restrict" {
        powershell -ExecutionPolicy Bypass -File scripts/smoke-queue-restriction.ps1 -Backend $backend
        break
    }
    "qa-full" {
    ...
    default {
        throw "Unknown scenario: $Scenario (expected smoke|reviewer|ban|gateway|queue-auth|retention|policy-hash|security-events|security-alerts|security-alerts-manual|session-revoke|auto-review|action-ack|queue-restrict|qa-fast|qa-full)"
    }
}
