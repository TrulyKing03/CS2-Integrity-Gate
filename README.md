...
4. Optional hardening: launcher signs session payload into `runtime/session.sig`.
...
- Optional runtime session signature validation before AC consumes `session.json`.
- Shared runtime file output:
  - `runtime/session.json` (input from launcher)
  - `runtime/session.sig` (optional launcher signature input)
...
- Optional runtime session signing (`session.sig`) when runtime signing key is configured.
- Waits for a valid join token generated by AC.
- Sends queue bearer token when available (from login response or `--access-token`) for optional queue auth enforcement.
...
- `ApiAuth.RequireQueueAccessToken`
- `ApiAuth.AccessTokenTtlMinutes`
...
- `CS2IG_RUNTIME_SIGNING_KEY` (launcher + AC runtime session signature)
...
powershell -ExecutionPolicy Bypass -File scripts/run-launcher.ps1 -Command play -RuntimeSigningKey "replace-with-shared-secret"
```
...
4. Configure the same runtime signing key in launcher + AC so session file tampering is rejected.
...
- Run `clear-runtime`, restart launcher + AC, and regenerate a fresh signed session.
